...
class RecyclerViewActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<RecyclerView>(R.id.recycler_view).apply {
            val data = deserializationData().filter { it.error == null && it.unknownError == null }
            adapter = TweetAdapter(data)
        }
    }

    private fun deserializationData() : List<Tweet> {
        val gson = Gson()
        val type = object : TypeToken<List<Tweet>>() {}.type
        return gson.fromJson(tweetData, type)
    }

    companion object {
        private const val tweetData = "[\n" +
                "  {\n" +
                "    \"content\": \"沙发！\",\n" +
                "    \"images\": [\n" +
                "      {\n" +
